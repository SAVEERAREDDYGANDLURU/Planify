...
//this was the code to add week and weekend opening and closing times category-wise.
/*
for(let i=0;i<dataset.length;i++)
{
    switch(dataset[i].category)
    {
        case "hindu_temple":
   {
       dataset[i]["week_open_time"]=["06:30","16:00"];
       dataset[i]["week_close_time"]=["12:00","21:00"];
       dataset[i]["weekend_open_time"]=["6:30"];
       dataset[i]["weekend_close_time"]=["13:00"];
   }
    break;
    case "fort":
        {
            dataset[i]["week_open_time"]=["09:00"];
            dataset[i]["week_close_time"]=["17:00"];
            dataset[i]["weekend_open_time"]=["09:00"];
            dataset[i]["weekend_close_time"]=["17:00"];
        }
         break;
    case "adventure&game":
        {
            dataset[i]["week_open_time"]=["11:00"];
            dataset[i]["week_close_time"]=["23:00"];
            dataset[i]["weekend_open_time"]=["11:00"];
            dataset[i]["weekend_close_time"]=["23:00"];
        }
         break;
         case "amusement_park":
   {
       dataset[i]["week_open_time"]=["10:00"];
       dataset[i]["week_close_time"]=["18:00"];
       dataset[i]["weekend_open_time"]=["10:00"];
       dataset[i]["weekend_close_time"]=["18:00"];
   }
    break;
    case "park":
        {
            dataset[i]["week_open_time"]=["09:00"];
            dataset[i]["week_close_time"]=["17:30"];
            dataset[i]["weekend_open_time"]=["09:00"];
            dataset[i]["weekend_close_time"]=["17:30"];
        }
         break;
         case "church":
   {
       dataset[i]["week_open_time"]=["07:00"];
       dataset[i]["week_close_time"]=["19:00"];
       dataset[i]["weekend_open_time"]=["07:00"];
       dataset[i]["weekend_close_time"]=["19:00"];
   }
    break;
    case "shopping_mall":
        {
            dataset[i]["week_open_time"]=["10:00"];
            dataset[i]["week_close_time"]=["22:00"];
            dataset[i]["weekend_open_time"]=["10:00"];
            dataset[i]["weekend_close_time"]=["22:00"];
        }
         break;
         case "movie_theater":
   {
       dataset[i]["week_open_time"]=["09:00"];
       dataset[i]["week_close_time"]=["22:30"];
       dataset[i]["weekend_open_time"]=["09:00"];
       dataset[i]["weekend_close_time"]=["22:30"];
   }
    break;
    case "beach":
        {
         ...
         case "mosque":
   {
    ...
    case "restaurant":
        {
            dataset[i]["week_open_time"]=["12:00"];
            dataset[i]["week_close_time"]=["23:30"];
            dataset[i]["weekend_open_time"]=["12:00"];
            dataset[i]["weekend_close_time"]=["23:30"];
        }
         break;
         case "museum":
   {
    ...
    case "zoo":
        {
            dataset[i]["week_open_time"]=["08:00"];
            dataset[i]["week_close_time"]=["17:30"];
            dataset[i]["weekend_open_time"]=["08:00"];
            dataset[i]["weekend_close_time"]=["17:30"];
        }
         break;
...
const fs = require('fs');
fs.writeFile("final_dataset.json", jsonContent, 'utf8', function (err) {
    if (err) {
...
var defaultKey = "677a4e77-bd25-4be9-83b5-9ae0ffdddfcc";
